...
const messageForUtsavMember = async (req, res) => {
  try {
    ...
    const message =
      "CONGRATS!! Your FINAFID Utsav membership is active. Now enjoy special discounts and benefits anytime you shop at FINAFID";
    const responseDetails = await sendSMS(
      message,
...
const messageForOrderDelivary = async (req, res) => {
  try {
    ...
    const message =
      "Exciting update! Your FINAFID Order has been Delivered.Enjoy your products.Thank you for shopping with FINAFID.";
    const responseDetails = await sendSMS(
      message,
      req.body.phoneNumber,
      templateId
